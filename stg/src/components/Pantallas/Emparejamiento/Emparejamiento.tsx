...
function Emparejamiento() {
  ...
  // Drag & Drop handlers con swap completo de campos
  const handleDragStart = (start: DragStart) => { setDraggingId(start.draggableId); setHighlightedId(null); };
  const handleDragUpdate = (update: DragUpdate) => {
    if (!update.destination) return;
    const idx = update.destination.index;
    const row = Math.floor(idx / 2);
    const col = idx % 2;
    setHighlightedId(`${row}-${col}`); // Resaltar celda destino
              ...
              {(provided) => (
                <tbody ref={provided.innerRef} {...provided.droppableProps}>
                  {filtered.map((fila, rowIndex) => (
                    <tr key={rowIndex}>
                      <td>{fila.tutor}</td>
                      <td><select value={fila.materiaTutor} onChange={(e) => actualizarTutor(rowIndex, "materiaTutor", e.target.value)} style={{ padding: "6px" }}><option value="">Vacío</option><option value="Ingles">Inglés</option><option value="Matematicas">Matemáticas</option></select></td>
                      <td><select value={fila.disponibilidadTutor} onChange={(e) => actualizarTutor(rowIndex, "disponibilidadTutor", e.target.value)} style={{ padding: "6px" }}>{disponibilidadOptions.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}</select></td>
                      {[fila.tutorado1, fila.tutorado2].map((_, colIdx) => {
                        ...
                        return (
                          <td key={cellId} className={`drop-target ${highlightedId === cellId ? "highlight" : ""}`}>
                            <Draggable draggableId={cellId} index={rowIndex * 2 + colIdx}>
                              {(prov) => (
                                <div ref={prov.innerRef} {...prov.draggableProps} {...prov.dragHandleProps} className={`draggable ${item.color} ${draggingId === cellId ? "dragging" : ""}`}>
                                  {isEmpty ? (<div className="tutorado-vacio">VACÍO</div>) : (
